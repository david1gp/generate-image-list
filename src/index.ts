...
export async function generateImageList(
  ...
  await writeGeneratedImagesFile(sorted, outputPath, imageTypeImportPath);
  await formatGeneratedImagesCodeFile(outputPath);
}

async function writeGeneratedImagesFile(
  imageMap: Record<string, ImageType>,
  outputPath: string,
  imageTypeImportPath: string,
): Promise<void> {
  const outputContent = `
  import type { ImageType } from "${imageTypeImportPath}"
...
`;
  ...
  console.log(
    `Generated ${Object.keys(imageMap).length} images to ${outputPath}`,
  );
}
